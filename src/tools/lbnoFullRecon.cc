...
class Reconstructor : public LbnoPlotter 
{
 public:
  Reconstructor() {useMagFieldFromCard = true;}
...
void Reconstructor::initDataCards() 
{
...
void Reconstructor::loadDataCards() 
{
...
void Reconstructor::reconstruct() 
{
  ...
  for(int j = 0;j<runs;j++)
  {
	...
	///		PI ZERO RECON			 ////
	/////////////////////////////////////////////////////
	double totalPiZeroReconEnergy = 0.;
	...
	if(!justTPC_){
	  ...
	  totalPiZeroReconEnergy += piZeroMgr->GetTotalEnergy(piZeroTrackVector,scintResolution_);
	  totalReconNuEnergy += totalPiZeroReconEnergy;

  	  tracking_piZero_truthVsReconEnergy_all_h2d.Fill(totalPiZeroTruthEnergy,totalPiZeroReconEnergy);
...
bool Reconstructor::isCCQE(NeutrinoEvent* nu_event){
...
bool Reconstructor::isBadEvent(double reconRatio,double cutOff){
	if ( (reconRatio < (1. - cutOff) ) || (reconRatio > (1. + cutOff) ) )return true;
	else return false;
}

void Reconstructor::showBadEvents(){
...
void Reconstructor::insertAscending(ReconData neutrino,std::vector<ReconData> &neutrinoData){
...
void Reconstructor::calculateMeanAndErrors(std::vector<ReconData> &neutrinoData,std::vector<double> &meanVector,std::vector<double> &errorVector){
