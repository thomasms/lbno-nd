...
class myReconstructor : public LbnoPlotter {
 ...
 private:
  ...
  TH2F tracking_piZero_truthVsReconEnergy_all_h2d;
  ...
  TH2F tracking_dEdxVsE_h2d;
  TH2F tracking_dEdxVsE_muon_h2d;
  TH2F tracking_dEdxVsE_pion_h2d;
  TH2F tracking_dEdxVsE_proton_h2d;
  TH2F tracking_dEdxVsE_electron_h2d;
...
void myReconstructor::initDataCards() {
  ...
  cards_->AddDataCardDouble("piZeroMassCut", 0.1*PIZEROMASS); //in MeV - number of sigma
...
void myReconstructor::initHistos(){
  ...
  tracking_piZero_truthVsReconEnergy_all_h2d.SetNameTitle("tracking_piZero_truthVsReconEnergy_all_h2d","tracking_piZero_truthVsReconEnergy_all_h2d");
  tracking_piZero_truthVsReconEnergy_all_h2d.SetBins(binning*2, 0.,maxEnergy_, binning*6,0.,maxEnergy_*3);
  ...
  tracking_dEdxVsE_h2d.SetNameTitle("tracking_dEdxVsE_h2d","tracking dE/dx vs energy histogram");
  tracking_dEdxVsE_h2d.SetBins(1000,-2.,2.,1000,0.,4.);
  tracking_dEdxVsE_muon_h2d.SetNameTitle("tracking_dEdxVsE_muon_h2d","tracking dE/dx vs energy histogram - muon");
  tracking_dEdxVsE_muon_h2d.SetBins(1000,-2.,2.,1000,0.,4.);
  tracking_dEdxVsE_pion_h2d.SetNameTitle("tracking_dEdxVsE_pion_h2d","tracking dE/dx vs energy histogram - pion");
  tracking_dEdxVsE_pion_h2d.SetBins(1000,-2.,2.,1000,0.,4.);
  tracking_dEdxVsE_proton_h2d.SetNameTitle("tracking_dEdxVsE_proton_h2d","tracking dE/dx vs energy histogram - proton");
  tracking_dEdxVsE_proton_h2d.SetBins(1000,-2.,2.,1000,0.,4.);
  tracking_dEdxVsE_electron_h2d.SetNameTitle("tracking_dEdxVsE_electron_h2d","tracking dE/dx vs energy histogram - electron");
  tracking_dEdxVsE_electron_h2d.SetBins(1000,-2.,2.,1000,0.,4.);
...
//	std::cout << "\nBacktracer: " << backTracer_[0] << ", " << backTracer_[1] << ", " << backTracer_[2] <<std::endl;
	...
	double totalPiZeroTruthEnergy = 0.;

	//loop over primaries to get truth piZero energy
	for(int fssIndex = 0;fssIndex<fssVector.size();fssIndex++){
		if(fssVector[fssIndex].getPDG() == 111)totalPiZeroTruthEnergy +=fssVector[fssIndex].getP4().E() * (1./CLHEP::GeV);
	}
	...
	if(verbose_>1 && verbose_<10 && ccqe){
		std::cout << "\n===== CCQE ======";
		std::cout << "\n--- Lepton --- \n";
	...
	double totalTransMomentumSquared = 0.;
	...
	if(tpcHitCount>0){
		...
		for(std::vector< HitCollection >::iterator k = tpcTrackVector.begin();k != tpcTrackVector.end();++k){
		  ...
		  double trackTruthTransMomentum = smearer.getTruthMomentum(tempVtr);
		  ...
		  totalTransMomentumSquared += trackTransMomentum*trackTransMomentum;

		  totalReconNuEnergy +=trackTransMomentum;
		  ...
		  if(numberOfHits>1){
			...
			if(range>0. && (numberOfHits > hitCountCondition_) ){
				double logEnergy = TMath::Log10(initEnergy);
				double logEDepPerLength = TMath::Log10(eDep/(range*100.*0.035)); // must be in MeV/cm and need to divide by density of argon at 20 bar
				tracking_dEdxVsE_h2d.Fill(logEnergy,logEDepPerLength);
				//tracking_dEdxVsE_h2d.Fill(initEnergy,eDep/range);
				...
				if(fssPDG==13){
					tracking_dEdxVsE_muon_h2d.Fill(logEnergy,logEDepPerLength);
				...
				if(fssPDG==2212){
					tracking_dEdxVsE_proton_h2d.Fill(logEnergy,logEDepPerLength);
				...
				if(fssPDG==211 || fssPDG==-211)	tracking_dEdxVsE_pion_h2d.Fill(logEnergy,logEDepPerLength);
				if(fssPDG==11)	tracking_dEdxVsE_electron_h2d.Fill(logEnergy,logEDepPerLength);
				if(ccqe){
	...
	//convert hits into vector of tracks to make it easier to reconstruct each one
	std::vector<ScintHitCollection> piZeroTrackVector = trackMgr->PiZeroPhotonTracks(scintSdHits); //tracker.PiZeroTracks(scintSdHits,prims);
	...
	if(scintHitCount>0){
	  ...
	  //skip events with incorrect photons tracks
	  if( photonTracksInEvent > (piZerosInEvent*2) )photonTracksInEvent=0;

	  std::vector<TLorentzVector> photonKinematics = piZeroMgr->SortPhotons(piZeroTrackVector,eventVertex,piZeroMassCutValue_*(1./CLHEP::GeV));
	                          
          while( photonKinematics.size() > k+1 ){  

		std::pair<TLorentzVector,TLorentzVector> photonPairKinemat;
		photonPairKinemat.first = photonKinematics.at(k);
		photonPairKinemat.second = photonKinematics.at(k+1);

               	double mass = piZeroMgr->ReconstructPiZeroInvariantMass(photonPairKinemat);
		mass *= 1000.; //mass from GeV to MeV

		if(mass==0.0)continue;

                //else find reconstructed energy of the photon
                double piZeroEnergy = piZeroMgr->ReconstructPiZeroEnergy(photonPairKinemat);
                totalPiZeroEnergy +=piZeroEnergy;
		...
		double angle = piZeroMgr->CalculateAngleBetweenPhotons(photonPairKinemat);  
                ...
                if(verbose_==10){
                  std::cout << "\n-------------------------------------------------------------------"
                            ...
                            << "\nPi0 recon energy: " << piZeroEnergy << ", total recon energy (before pi0): " << totalReconNuEnergy
                            << "\nTotal recon energy (after pi0): " << totalReconNuEnergy+piZeroEnergy << " truth nu energy: " << nuMom
                            << "\n-------------------------------------------------------------------";
                }
                if(mass>0){
                ...
                if( piZeroEnergy>0. ){
                  reconPiZeroCount++; 
                  totalPiZeroEnergy +=piZeroEnergy;
                  totalReconNuEnergy +=piZeroEnergy;
...
/*
	  //loop over pi zeros
	  for(int k =0;k<piZeroTrackVector.size();k+=2){
	  	//check the invariant mass
		std::pair<TLorentzVector,TLorentzVector> photonPairKinemat;
		photonPairKinemat.first = piZeroMgr->ReconstructPhotonKinematics(piZeroTrackVector.at(k),eventVertex);
		photonPairKinemat.second = piZeroMgr->ReconstructPhotonKinematics(piZeroTrackVector.at(k+1),eventVertex);
...
*/
	  tracking_piZero_truthVsReconEnergy_all_h2d.Fill(totalPiZeroTruthEnergy,totalPiZeroEnergy);
	...
	//totalTransMomentum = TMath::Sqrt(totalTransMomentumSquared);

	if(totalReconNuEnergy>0. && goodEvent &&
	  momentumMinSelect_ < nuMom && momentumMaxSelect_ > nuMom){

		ReconData reconstructedNeutrinoEvent(backTracer_[1],nuMom,totalReconNuEnergy);
		...
		if(ccqe){
			...
			if( isBadEvent(ratio,reconstructionCut_) ){
				ReconData tempData(i,nuMom,totalReconNuEnergy);
		...
		tracking_recon_mom_h1d.Fill(totalReconNuEnergy);
		tracking_reconDiff_mom_h1d.Fill( (nuMom-totalReconNuEnergy)/nuMom );
		tracking_truthVsRecon_mom_h2d.Fill(nuMom,totalReconNuEnergy);
		tracking_truthVsDiff_mom_h2d.Fill( nuMom,((nuMom-totalReconNuEnergy)/nuMom) );
	}//end tpc recon
	...
	if(verbose_>1 && verbose_<10){
        	std::cout << "\n ================================================================ "
			  ...
			  << "\n Scint Hits: " << scintHitCount
		  	  ...
		  	  << "\n Total Reconstructed Momentum [GeV/c]: " << totalReconNuEnergy
...
void myReconstructor::write(){
  ...
  tracking_piZero_truthVsReconEnergy_all_h2d.Write();
  ...
  tracking_dEdxVsE_h2d.Write();
  tracking_dEdxVsE_muon_h2d.Write();
  tracking_dEdxVsE_pion_h2d.Write();
  tracking_dEdxVsE_proton_h2d.Write();
  tracking_dEdxVsE_electron_h2d.Write();
...
void myReconstructor::makeGraphs(){
  ...
  //fill the x axis
  for(int xIndex=0;xIndex<numberOfBins;xIndex++){
