...
class myFlux : public LbnoPlotter {
 public:
  myFlux() {}
  virtual ~myFlux() {}
 ...
 private:
  ...
  TH1F gammaKinEnergy_intoTpc_h1d_;
  TH1F electronKinEnergy_intoTpc_h1d_;
  TH1F muonKinEnergy_magnetOnly_h1d_;
  TH1F muonKinEnergy_rockOnly_h1d_;
  TH1F gammaKinEnergy_TPConly_h1d_;
  ...
  int inFluxNeutrinoCount,outFluxNeutrinoCount;
...
void myFlux::initHistos(){
  ...
  gammaKinEnergy_intoTpc_h1d_.SetNameTitle("gammaKinEnergy_intoTpc_h1d_","gammaKinEnergy_intoTpc_h1d_");
  gammaKinEnergy_intoTpc_h1d_.SetBins(binning,0.,maxEnergy_);
  electronKinEnergy_intoTpc_h1d_.SetNameTitle("electronKinEnergy_intoTpc_h1d_","electronKinEnergy_intoTpc_h1d_");
  electronKinEnergy_intoTpc_h1d_.SetBins(binning,0.,maxEnergy_);

  gammaKinEnergy_TPConly_h1d_.SetNameTitle("gammaKinEnergy_TPConly_h1d_","gammaKinEnergy_TPConly_h1d_");
  gammaKinEnergy_TPConly_h1d_.SetBins(binning,0.,maxEnergy_);
  ...
  nodePositionXZ_h2d_.SetNameTitle("nodePositionXZ_h2d_","nodePositionXZ_h2d_");
  ...
  muon_range_h1d_.SetNameTitle("muon_range_h1d_","muon_range_h1d_");
  muon_range_h1d_.SetBins(1000,0,60.);
...
*/
    //volume nodes
    GeomVolumeRecord geomVolumeCounts,geomVolumeCountsFromInParticles,geomVolumeCountsFromOutParticles;
...
//	  std::cout << "\nnuEvent Energy: " << nuEvent_->getNuEnergy()*(1./CLHEP::GeV)	<< "\tmaxEnergy_: " << maxEnergy_;

	  if(nuEvent_->getNuEnergy()*(1./CLHEP::GeV) > maxEnergy_)continue;
	  ...
	  for(int l=0;l<ins->size();l++){
		...
		TLorentzVector node_pos_in = ins->at(l).getPosition(); //mm
		...
		inCount++;
		if(fInPDG==14 || fInPDG==-14 || fInPDG==12 || fInPDG==-12)inFluxNeutrinoCount++;
		if(fInPDG==13)inFluxMuonCount++;
        	if(fInPDG==11)
          {
            inFluxElecCount++;
            electronKinEnergy_intoTpc_h1d_.Fill(kinenergy);
          }
        	if(fInPDG==22)
          {
            inFluxGammCount++;
            gammaKinEnergy_intoTpc_h1d_.Fill(kinenergy);
          }
		...
		if(fInPDG==13){
		  ...
		  double range_muon = TMath::Sqrt( TMath::Power(nodePos.X() -node_pos_in.X() ,2)
						 + TMath::Power(nodePos.Y() -node_pos_in.Y() ,2) 
						 + TMath::Power(nodePos.Z() -node_pos_in.Z() ,2) );
		  muon_range_h1d_.Fill(range_muon/1e3); //in m
		}
	  ...
	  for(int l=0;l<outs->size();l++){
		...
		TLorentzVector node_pos = outs->at(l).getParent().getPosition(); //mm
		std::string nodeName = nuEvent_->getNodeName();
		geomVolumeCountsFromOutParticles.findVolume(nodeName);
		bool tpcInteraction = geomVolumeCountsFromOutParticles.isTpc();

		if(tpcInteraction)if(fOutPDG==22)gammaKinEnergy_TPConly_h1d_.Fill(fOutKEAtVertex*(1./1000.)); //convert to GeV		

		if(!tpcInteraction)continue;
		...
		else if(fOutPDG==14 || fOutPDG==-14 || fOutPDG==-12 || fOutPDG==12)outFluxNeutrinoCount++;
...
void myFlux::print(){
  ...
  std::cout  << "\n================================================================"
   ...
   std::cout << "\n================================================================"
	     ...
	     << "\n**** Neutrinos:"
             << "\n\t\tIn:\t\t\t" << inFluxNeutrinoCount << std::setprecision(3) << " (" << inFluxNeutrinoCount*100./inCount<< "%)"
             << "\n\t\tOut:\t\t\t" << outFluxNeutrinoCount << std::setprecision(3) << " (" << outFluxNeutrinoCount*100./outFluxAllParticleCount<< "%)"
...
void myFlux::write(){
  ...
  gammaKinEnergy_intoTpc_h1d_.SetLineWidth(2);
  gammaKinEnergy_intoTpc_h1d_.Write();
  electronKinEnergy_intoTpc_h1d_.SetLineWidth(2);
  electronKinEnergy_intoTpc_h1d_.Write();

  gammaKinEnergy_TPConly_h1d_.SetLineWidth(2);
  gammaKinEnergy_TPConly_h1d_.Write();
