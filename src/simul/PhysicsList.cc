...
PhysicsList::PhysicsList(std::string hadlist,std::string emlist) : G4VUserPhysicsList(), defaultCut_(0.), gammaCut_(0.), electronCut_(0.), positronCut_(0.), neutronCut_(0.), cutEM_(0.)
{
  hadlist_ = hadlist;
  emlist_ = emlist;
}
...
void PhysicsList::initialize(){
  ...
  if(defaultCut_>0) defaultCutValue = defaultCut_;
  else defaultCutValue = 0.1*mm;

  if(gammaCut_>0) cutForGamma = gammaCut_;
  else cutForGamma = 0.1*mm;

  if(electronCut_>0) cutForElectron = electronCut_;
  else cutForElectron = 0.1*mm;

  if(positronCut_>0) cutForPositron = positronCut_;
  else cutForPositron = 0.1*mm;

  if(neutronCut_>0) cutForNeutron = neutronCut_;
  else cutForNeutron = 0.1*mm;
  ...
  std::cout << "\nPhysicsList::initialize() - cuts: "
	    ...
	    << "\n\tcutForPositron = " << cutForPositron/CLHEP::mm  << " mm"<< std::endl;
